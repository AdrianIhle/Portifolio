...
void GameWidget::initializeGL()
{
    ...
    m_terning->initVertexBufferObjects();
    m_terning->setTexture(m_texture->id());;        //krever dobble semikolon for å rendre av en eller annen grunn
    m_gulv->initVertexBufferObjects();
    m_gulv->setTexture(m_texture->id());;
    m_hjul->initVertexBufferObjects();
    m_hjul->setTexture(m_texture->id());;
    m_ball->initVertexBufferObjects();
    m_ball->setTexture(m_texture->id());;
...
void GameWidget::klikket()
{
    ...
    {
//        bool ok;
//        double x = QInputDialog::getDouble(this, tr("Koordinater for ")+navn,
//                                                   tr("x:"), 1.0, -10, 10, 2, &ok);
//        double y = QInputDialog::getDouble(this, tr("Koordinater for ")+navn,
//                                                   tr("y:"), 1.0, -10, 10, 2, &ok);
//        double z = QInputDialog::getDouble(this, tr("Koordinater for ")+navn,
//                                                   tr("z:"), 1.0, -10, 10, 2, &ok);
        terningPaa = true;
    }
...
void GameWidget::paintGL()
{
    ...
    m_gulv->draw(m_positionAttribute, m_colorAttribute, m_textureAttribute);
    ...
    // Disse objektene blir påvirket av rotasjonen
    if(tetraederPaa)
        m_tetraeder->draw(m_positionAttribute, m_colorAttribute, m_textureAttribute);
    if(terningPaa)
        m_terning->draw(m_positionAttribute, m_colorAttribute, m_textureAttribute);
    if(hjulPaa)
        m_hjul->draw(m_positionAttribute, m_colorAttribute, m_textureAttribute);
    if(ballPaa)
        m_ball->draw(m_positionAttribute, m_colorAttribute, m_textureAttribute);
