...
AudioSource::AudioSource(GameObject *owner, std::string name, bool loop, float gain) : Component(owner),
    mName(name),
    mSource(0),
    mBuffer(0),
...
bool AudioSource::loadWave(std::string filePath)
{
...
void AudioSource::setPosition(Vec3 newPos)
{
    mPosition = newPos;
    ALfloat temp[3] = {mPosition.x, mPosition.y, mPosition.z};
    alSourcefv(mSource, AL_POSITION, temp);
}
void AudioSource::setVelocity(Vec3 newVel)
{
    mVelocity = newVel;
...
bool AudioSource::checkError(std::string name)
{
    switch (alGetError())
    {
    case AL_NO_ERROR:
